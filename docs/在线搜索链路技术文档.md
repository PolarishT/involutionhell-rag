...
- Query Harness：`src/main/java/com/involutionhell/backend/rag/retrieval/harness/DefaultRagQueryHarness.java`
...
sequenceDiagram
    ...
    participant Harness as RagQueryHarness
    ...
    Ask->>Harness: execute(query context)
    Harness->>Transform: transform(question, history)
    Transform-->>Harness: retrievalQuestion
    Harness-->>Ask: TRANSFORMED
    Ask-->>Controller: query_transformed
    Harness->>Expand: expand(retrievalQuestion)
    Expand-->>Harness: retrievalQueries
    Harness-->>Ask: EXPANDED
    Ask-->>Controller: query_expanded
    loop each retrieval query
        Harness->>Retriever: search(query, perQueryTopK, filter)
        Retriever->>Indexing: read searchable chunks
        Indexing-->>Retriever: chunk views
        Retriever-->>Harness: scored chunks
    end
    Harness->>Joiner: RRF join(query results, topK)
    Joiner-->>Harness: topK contexts
    Harness->>Indexing: expand neighbor window
    Indexing-->>Harness: adjacent active chunks
    Harness-->>Ask: CONTEXTS
...
2. `RagAskService` 将 query 上下文交给 `RagQueryHarness`，并把 Harness 阶段事件翻译为对外 SSE。
3. `RagQueryTransformer` 先按配置和阈值决定是否使用 Spring AI `CompressionQueryTransformer` 压缩多轮问题，再用 `RewriteQueryTransformer` 改写检索 query。
4. `RagQueryExpander` 使用 Spring AI `MultiQueryExpander` 生成多路检索 query；组件不可用或失败时回退为原始 query。
5. 每个 query 通过 WebFlux `Flux.flatMap` 并发调用当前注入的 `RagRetriever`，阻塞调用隔离到 `ragBlockingScheduler`。
6. 多 query 结果经 `RagDocumentJoiner` 使用 RRF 去重、融合、重排。
7. `RagQueryHarness` 基于 `neighborWindowBefore/After` 补齐命中 chunk 前后的 active generation 邻居片段，并产出 `CONTEXTS` 阶段事件。
8. `RagAnswerGenerator` 使用 `ChatClient.stream().content()` 生成 `answer_delta`；没有上下文、没有 `ChatModel` 或调用失败时回退为本地摘要。
...
| `rag.query-transformation.rewrite-confidence-enabled` | `true` | 是否在 rewrite 前进行原 query 可检索置信度判定 |
| `rag.query-transformation.rewrite-confidence-threshold` | `0.65` | 原 query 置信度低于该值才触发 rewrite |
| `rag.query-transformation.rewrite-confidence-timeout-millis` | `1000` | rewrite 置信度判定超时时间，超时后跳过 rewrite |
...
| `rag.retrieval.query-timeout-millis` | `3000` | 单个扩展 query 的总检索超时 |
...
- Query Harness 阶段流：`TRANSFORMED`、`EXPANDED`、`CONTEXTS`。
...
- Query Harness 目前是固定阶段流，尚未开放 middleware / hook 扩展点。
- `RagRequestFeedbacks` 已接入主链路，`notice` 与 `completed.degraded/notices` 会反映真实降级。
- answer generation 缺少与 query harness 一致的阶段事件封装。
